        ...
        <q-card-section>
          <div class="row">
            <div class="col q-ma-sm">
              <q-input dense :readonly="isUpdateMode" v-model="itemProxy.username" :label="$t('users.table.username')" :rules="rules.loginUsername"/>
            </div>
            <div class="col q-ma-sm">
          ...
          <div class="row">
            <div class="col q-ma-sm">
              <q-input dense v-model="itemProxy.loginFailureCount" type="number" :label="$t('users.table.loginFailureCount')" :rules="[PositiveNumber()]"/>
            </div>
            <div class="col q-ma-sm">
              <q-select dense options-dense map-options emit-value v-model="itemProxy.enabled" :options="vs.enabledOptions" :label="$t('common.use')" :rules="[Choose()]"/>
            </div>
          </div>
          <div class="row">
            <div class="col q-ma-sm">
              <q-select dense options-dense map-options emit-value v-model="itemProxy.roleGroupId" :options="roleGroupOptions" option-label="roleGroupName" option-value="roleGroupId" :label="$t('users.table.roleGroupName')" :rules="[Choose()]"/>
            </div>
          </div>
        </q-card-section>
...
import rules, {Required, PositiveNumber, Between, NullOrMin, Choose} from 'utils/validationRules'
import api from '@/api'
...
function onBeforeShow() {
  ...
  itemProxy.value = props.item ? JSON.parse(JSON.stringify(props.item)) : {loginFailureCount:0, enabled: 'YES'} // object deep copy required.
